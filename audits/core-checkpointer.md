...
| 2 | `core/checkpointer.py` | 195–210 | **ALTA** | Falha de atomicidade e mascaramento silencioso de falhas de rollback em `execute_time_travel`. O método monta duas queries separadas (`DELETE FROM fsm_checkpoints` e `UPDATE files`) e as executa iterando sobre `write_fn(q, params)` sem transação envolvente (`BEGIN IMMEDIATE`). Além disso, `write_fn` (`execute_write`) intercepta erros e retorna uma tupla `(False, error)`. O loop ignora esse retorno e nunca lança exceção. O bloco `except` é inútil e o método retorna `target_data`, reportando falso sucesso enquanto o rollback falhou completamente no SQLite. Conforme descoberto em `schema-oficial-incompleto.md`, as tabelas `fsm_checkpoints` e `files` nunca existem numa instalação limpa, tornando essa falha e falso sucesso 100% garantidos em produção. | Simulação de falha no banco em `execute_write`: método retornou `target_data` com sucesso, mascarando 100% da falha de rollback. Em instalação limpa, a ausência de DDL garante a falha em todo rollback. | **CONFIRMADO** |
...
- **Adendo Pós-Descoberta de `schema-oficial-incompleto` (Garantia de Falha Silenciosa em Toda Instalação Limpa):**  
  A auditoria transversal de [`audits/schema-oficial-incompleto.md`](file:///c:/Nexus-Memory/GrafoConcierge/audits/schema-oficial-incompleto.md) provou que **as tabelas `fsm_checkpoints` e `files` nunca são criadas pelo bootstrap oficial de produção** (`storage/schema.py`). Isso eleva a gravidade do Achado #2 de uma "falha condicional ou ocasional sob erros de banco" para uma **certeza estrutural determinística**: em qualquer instalação limpa do Grafo Concierge, toda e qualquer chamada a `execute_time_travel` **falha obrigatoriamente** em ambas as queries (`OperationalError: no such table: fsm_checkpoints` e `OperationalError: no such table: files`). Pelo mecanismo deste Achado #2, como `execute_write` engole as exceções retornando `(False, OperationalError)` e `execute_time_travel` descarta cegamente o retorno de `write_fn`, o método invariavelmente retorna `target_data` com falso sucesso total, garantindo 100% de mascaramento silencioso em produção sem nunca ter realizado um único rollback físico.

- **Impacto no Sistema:**  
  Falso positivo de Time-Travel: o orquestrador e os agentes assumem que o rollback ocorreu com sucesso, enquanto o banco SQLite permanece em estado corrompido ou inconsistente. Em instalações limpas, a falha é permanente e 100% invisível.
- **Correção Conceitual Sugerida (Fase 3):**  
