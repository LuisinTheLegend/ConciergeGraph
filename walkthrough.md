...
| 3 | **GRAVE** | [`storage/store.py:107`](file:///c:/Nexus-Memory/GrafoConcierge/storage/store.py#L107), [`core/database.py:66–67`](file:///c:/Nexus-Memory/GrafoConcierge/core/database.py#L66) e [`interface/watcher.py:93–100`](file:///c:/Nexus-Memory/GrafoConcierge/interface/watcher.py#L93) — **Mascaramento Silencioso de Falhas DDL e Falsa Confirmação no Boot**: `core/database.py::execute_write` engole exceções retornando `(False, error)`. `AgnosticCheckpointer.save_checkpoint` retorna `False` sem logar erro. `interface/watcher.py` silencia falhas em `files` com `except Exception: pass`. Agravando o cenário, `storage/store.py:107` loga `Schema v3.8.0 verified - all tables and triggers OK.` porque `verify_tables_exist()` valida apenas seu próprio subset restrito, entregando ao operador uma falsa confirmação de integridade no boot enquanto 5 tabelas críticas de `core/*` estão ausentes. |
