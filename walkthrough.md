...
> **Commit:** `a43ead4` (`docs(audit): core/vector_reconciler.py concluido`)  
...
| 4 | **MÉDIA** | [`core/vector_reconciler.py:48–50`](file:///c:/Nexus-Memory/GrafoConcierge/core/vector_reconciler.py#L48-L50) — Falta de paginação em lote e ausência de tratamento de exceções em `delete_batch`. Lista inteira de órfãos é enviada de uma só vez (ex: lote único de 500 itens), estourando limites de payload de rede (Qdrant) ou memória (Chroma). |
