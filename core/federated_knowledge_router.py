"""
core/federated_knowledge_router.py — SDD-SURVIVAL-22

Federated Knowledge Router.

Routes knowledge retrieval to the correct ecosystem based on the
intent classification provided by IntentClassifier:

  - LOCAL_CODEBASE   →  Private local GraphRAG (SQLite + Vector Backend)
...
EXTERNAL_FEDERATED_MCP = "EXTERNAL_FEDERATED_MCP"


class FederatedKnowledgeRouter:
    """Knowledge router delegating queries between local GraphRAG and federated MCPs."""
    ...
    def _resolve_local(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("FederatedKnowledgeRouter: Local GraphRAG failure: %s", e)
            local_context = f"[GraphRAG Error] Could not retrieve local context: {e}"

        logger.debug(
            "FederatedKnowledgeRouter: LOCAL resolution for '%s' (%d chars)",
            query[:60],
            len(str(local_context)),
        )
        return {
    ...
    def _resolve_external(self, query: str) -> Dict[str, Any]:
        """Queries external federated documentation servers."""
        external_context = ""
        ...
        if self.external_mcp:
            try:
                ...
                logger.warning(
                    "FederatedKnowledgeRouter: Federated MCP connection failure: %s", e
                )
                external_context = (
                    "Connection error with federated documentation server."
                )

        if not external_context:
            external_context = (
                f"[Federated Fallback] External market information regarding: {query}"
            )

        logger.debug(
            "FederatedKnowledgeRouter: EXTERNAL resolution for '%s' (%d chars)",
            query[:60],
            len(external_context),
        )
        return {
            "source": EXTERNAL_FEDERATED_MCP,
            "context": external_context,
            "is_private": False,
        }


# Domain transition alias
NozomioRouter = FederatedKnowledgeRouter
