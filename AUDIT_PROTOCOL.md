...
- [x] **bypass-governanca-por-session-id** — PRIORIDADE MÁXIMA (achado transversal de segurança; suplanta em gravidade qualquer item já concluído; ver [`audits/bypass-governanca-por-session-id.md`](file:///c:/Nexus-Memory/GrafoConcierge/audits/bypass-governanca-por-session-id.md)).
  ...
  1 CRÍTICA (falha estrutural de design: `session_id` e `agent_id` sem autenticação nem amarração de transporte, confirmada e reproduzida empiricamente com roubo de credenciais via `agent_get_checkpoint` e sobrescrita arbitrária de checkpoints da vítima via `agent_save_checkpoint`),
